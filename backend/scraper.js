...
function normalize({ name, price, imgSrc }) {
 const num =
  typeof price === "number" ? price : typeof price === "string" ? Number(price.replace(/[^\d.]/g, "")) || null : null
...
async function tryHeadless(url) {
 ...
 try {
  ...
  const product = await page.evaluate(() => {
   ...
   document.querySelectorAll('[class*="price"]').forEach((el) => priceCandidates.push(pickText(el)))
